...
2. **Khởi tạo & Chọn Lô Hàng (15s)**:
   ...
   - Mở lô kiểm thử hoàn chỉnh: `Sầu riêng Ri6 · DUR-260830-01` (354 km từ Đắk Lắk về TP.HCM).

3. **Hành Trình Chuỗi Cung Ứng & Chụp Ảnh Nguồn (20s)**:
   - Cho thấy 5 chặng thực tế: `Nhà vườn → Đóng gói → Kiểm định → Vận chuyển → Điểm bán`.
   - **Tính năng khóa ảnh nguồn**: Ảnh sản phẩm chụp tại vườn được gán mã băm SHA-256 và khóa cứng vào canonical event payload ngay khi ký xác nhận; không thể tráo ảnh sau khi đã ký.
...
4. **AI Đối Chiếu Chứng Từ Số (15s)**:
   ...
   - Nhãn minh bạch **DEMO EXTRACTION** trích xuất cấu trúc trường.
   - Hệ thống AI chạy rule kiểm toán đối chiếu:
     - **Matched**: Tỷ lệ hao hụt thực tế 10.0% khớp mức khai báo; khối lượng khớp dữ liệu chặng.
     - **Warning**: Cảnh báo khi phát hiện lệch mã lô hoặc sai thời gian thu hoạch so với kiểm nghiệm.
...
6. **Trải Nghiệm Quét QR Người Tiêu Dùng (10s)**:
   ...
   - Người mua xem toàn bộ timeline, ảnh nguồn đã khóa, đơn vị xác nhận từng chặng, và huy hiệu `CHECK-DI REGISTRY · 5/5 VERIFIED`.
